...
function SheetOverlay({
  ...
  return (
    <SheetPrimitive.Overlay
      ...
      className={cn(
        "fixed inset-0 z-50 bg-black/20 backdrop-blur-sm duration-200 data-open:animate-in data-open:fade-in-0 data-closed:animate-out data-closed:fade-out-0",
        className
      )}
      {...props}
    />
  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        ...
        data-side={side}
        className={cn(
          // Removed all border classes (e.g., border-r, border-l, border-t)
          // Added 'border-none' and 'shadow-2xl' for a cleaner look
          "fixed z-50 flex flex-col gap-4 bg-background bg-clip-padding text-sm shadow-2xl transition duration-300 ease-in-out border-none",
      ...
      >
        ...
        {showCloseButton && (
          <SheetPrimitive.Close data-slot="sheet-close" asChild>
            <Button
              variant="ghost"
              className="absolute top-4 right-4 rounded-full size-8"
              size="icon"
            >
